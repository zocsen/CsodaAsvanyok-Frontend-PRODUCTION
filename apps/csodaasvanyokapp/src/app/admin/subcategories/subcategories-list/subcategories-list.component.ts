...
export class SubcategoriesListComponent implements OnInit, OnDestroy {
  ...
  deleteSubcategory(subcategoryId: string) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.subcategoriesService.deleteSubcategory(subcategoryId).pipe(takeUntil(this.ngUnsubscribe)).subscribe({
          next: () => {
            this._getSubcategories();
  ...
  updateSubcategory(subcategoryId: string) {
    this.router.navigateByUrl(`admin/subcategories/form/${subcategoryId}`);
  }

  private _getSubcategories() {
    this.subcategoriesService.getSubcategories().pipe(takeUntil(this.ngUnsubscribe)).subscribe(cats => {
